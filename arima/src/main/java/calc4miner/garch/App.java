...
public class App 
{
    public static void pricePredictions()
    {
        try {
            ArrayList<Double> vals = new ArrayList<Double>();
            ArrayList<Double> timeVals = new ArrayList<Double>();
            BufferedReader reader = new BufferedReader(new FileReader("data/ethereum_price.csv"));
            int count = 0;
            ...
            File file = new File("data/price_predictions.csv");
            file.createNewFile();
    ...
    public static void statsPredictions()
    {
        try {
            ArrayList<Double> vals = new ArrayList<Double>();
            ArrayList<Double> timeVals = new ArrayList<Double>();
            BufferedReader reader = new BufferedReader(new FileReader("data/ethereum_stats.csv"));
            int count = 0;
            for(String line; (line = reader.readLine()) != null; count++) {
                timeVals.add((double)count);
                double val = Double.parseDouble(line.substring(line.indexOf(",") + 1));
            ...
            File file = new File("data/stats_predictions.csv");
            file.createNewFile();
            BufferedWriter writer = new BufferedWriter(new FileWriter(file));
            writer.write(String.format("%.20f\n", series[series.length - 1]));
            for (int i = 0; i < forecast_arr.length; i++) 
